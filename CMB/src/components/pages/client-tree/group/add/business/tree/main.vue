<template>
    <div>
        <Row :style="rowStyle" class="line">
            <div v-if="isShowOperate.showSelect">
                <Col :span="2" class="left-style">
                    <span>{{nodeType}}：</span>
                </Col>
                <Col :span="4" class="select-style">
                    <Select :data="auditorData" v-model="folder.val" :isNullSel="false"></Select>
                </Col>
            </div>
            
            <Col :span="2" :style="styleObject" class="child-style">
                <span>{{nodeText}}：</span>
            </Col>
            <Col :span="4">
                <Input :disabled="disabled" v-model="folder.name" :placeholder="placeholder" v-if="curLevel == 1"></Input>
                <div v-if="curLevel !== 1" class="node-style" @click="openDialog">{{folder.custName}}</div>
            </Col>
            <Col :span="2" class="right-operate" @click.native.stop="clickFlag ? addChild() : ''">
                <i class="add-icon"></i>
                <span class="childnode-text">添加子节点</span>
            </Col>
             <Col :span="2" class="right-operate" v-if="isShowOperate.showDelete" @click.native.stop="deleteChild">
...
<script type="text/javascript">
	export default{
		...
		methods:{
            // 添加节点
			addChild(){
                if (this.curLevel === 1 && typeof this.folder.name == 'undefined') {
                    this.$message.warning({"message": "请填写树名称"});
                    return;
                } else if (this.curLevel !== 1 && this.folder.val == undefined) {
                    this.$message.warning({"message": "请选择节点类型"});
                    return;
                }

                if (this.curLevel >= 7) {
                    this.$message.warning({"message": "最多只能添加7层"})
            ...
            // 打开弹窗
            openDialog () {
                if (this.folder.val == undefined) {
                    this.$message.warning({"message": "请选择设备号"});
                    return;
                }
                if (this.folder.val == '客户') {
