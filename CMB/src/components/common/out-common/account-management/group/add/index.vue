    ...
    <div class="addAcount">
        <Scroll>
                ...
                <Row>
                    <Form labelWidth="120px" labelAlign="left">
                        ...
                        <FormItem labelText="帐户区局：" :errorMessage="errors.first('firstbureau')">
                            <Select :isNullSel='true' :data="targetRcArr" name="firstbureau" data-vv-as="帐户区局" dataLabelStr="attrValueName" dataValueStr="attrValue" v-model="acctAddObj.firstbureau"></Select>
                        </FormItem>
                        </Col>
                        <Col :span="7" :offset="1">
                        ...
                        <Col :span="7" :offset="1">
                            <FormItem labelText="帐户状态：">
                                <Select
                                    ...
                                    name="accountStatusCd" v-model="acctAddObj.statusCd"></Select>
                            </FormItem>
                        </Col>
                ...
                <Row>
                    <Form labelWidth="120px" labelAlign="left">
                        ...
                        <FormItem labelText="帐单寄达地址：" :requiredIcon="false" :errorMessage="errors.first('contactAddr')">
                            <!-- <Input v-model.trim="acctAddObj.contactAddr" disabled @click.stop.native="showAddress()" data-vv-as="帐单寄达地址" name="contactAddr" v-validate="''"></Input> -->
                            <!-- <Input v-model.trim="acctAddObj.contactAddr" data-vv-as="帐单寄达地址" name="contactAddr" v-validate="''"></Input> -->
                            <Select name="contactAddr"
                                    data-vv-as="帐单寄达地址"
                                    :data="acctAddress"
                                    v-model.trim="acctAddObj.contactAddr"
                        ...
                        <FormItem labelText="帐单介质：" :requiredIcon="true" :errorMessage="errors.first('postMethod')">
                            <Select v-validate="'required'" :isNullSel='true' dataValueStr="attrValue" dataLabelStr="attrValueName" :data="postMethodArray" name="postMethod" data-vv-as="帐单介质" v-model="acctAddObj.postMethod"></Select>
                        </FormItem>
                        </Col>
            ...
            <div class="a-v-title">
                ...
                <span v-if="userRightBtnControl.paymentSchemeAdd" class="app__btn app__btn-add" @click.stop.self="addPaymentPlan">添加支付方案</span>
            </div>
                ...
                <Scroll class="app__table" :isShowBottom="true" fixedTopClassName="app__table-thead" ref="listScroll" fixedLeftClassName="app__table-left" fixedRightClassName="app__table-right">
                    ...
                    <Row :class="['app__table-item overflow-style']" :inline="true" v-for="(item,index) in addPaymentData" :key="index">
                        ...
                        <Col :width="100" class="app__table-right">
                            <span v-if="userRightBtnControl.paymentSchemeModify" @click="modPayment(index,item)">修改</span>
                ...
                <Row class="paymentplan">
                    <Form labelWidth="120px" labelAlign="left" data-vv-scope="addPlanObj">
                        ...
                        <FormItem labelText="付费方式：" :requiredIcon="true" :errorMessage="errors.first('addPlanObj.paymentMethod')">
                            <Select :data="paymentArray" :disabled="acctAddObj.acctBillingType == '40'" data-vv-as="付费方式" name="paymentMethod" :isNullSel='true' v-validate="'required'" dataValueStr="attrValue" dataLabelStr="attrValueName" v-model="addPlanObj.paymentMethod"></Select>
                        </FormItem>
                        </Col>
                ...
                <Row class="paymentplan">
                    <Form labelWidth="120px" labelAlign="left" data-vv-scope="modPlanObj">
                        ...
                        <FormItem labelText="付费方式：" :requiredIcon="true" :errorMessage="errors.first('modPlanObj.paymentMethod')">
                            <Select :data="paymentArray" :disabled="acctAddObj.acctBillingType == '40'" name="paymentMethod" data-vv-as="付费方式" :isNullSel='true' v-validate="'required'" dataValueStr="attrValue" dataLabelStr="attrValueName" v-model="modPlanObj.paymentMethod"></Select>
                        </FormItem>
                        </Col>

                         <Col :span="10" :offset="2">
                        <FormItem labelText="托收银行名称：" :requiredIcon="modPlanObj.paymentMethod == 110300" :errorMessage="errors.first('modPlanObj.payAcctName')">
                            <Input v-model="modPlanObj.extAcct.payAcctName" name="payAcctName" data-vv-as="托收银行名称" v-validate="modPlanObj.paymentMethod == 110300 ? 'required' : ''" maxlength="0" :disabled="addPlanObj.paymentMethod == 100000" :placeholder="modPlanObj.paymentMethod == 100000 ? '' : '请选择托收银行'" @click.stop.native="showPayAcctMod"></Input>
                        </FormItem>
                        </Col>

                        <Col :span="10" :offset="1">
                        <FormItem labelText="托收银行帐号：" :requiredIcon="modPlanObj.paymentMethod == 110300" :errorMessage="errors.first('modPlanObj.payAcctCode')">
                            <Input v-model="modPlanObj.extAcct.payAcctCode" name="payAcctCode" disabled v-validate="modPlanObj.paymentMethod == 110300 ? 'required|isBankNum' : 'isBankNum'" data-vv-as="托收银行帐号"></Input>
                        </FormItem>
                        </Col>

                        <Col :span="10" :offset="2">
                        <FormItem labelText="多方协议编号：" :errorMessage="errors.first('modPlanObj.eftAgreeNum')">
                            <Input v-model="modPlanObj.extAcct.eftAgreeNum"></Input>
                        </FormItem>
                        </Col>
                        ...
                        <FormItem labelText="交换号：" :errorMessage="errors.first('modPlanObj.exchgNum')">
                            <Input v-model="modPlanObj.extAcct.exchgNum" :disabled="modPlanObj.paymentMethod == 100000"></Input>
                        </FormItem>
                        </Col>

                        <Col :span="10" :offset="2">
                        <FormItem labelText="支付行号：" :requiredIcon="modPlanObj.paymentMethod == 110300" :errorMessage="errors.first('modPlanObj.paidBankNum')">
                            <Input disabled v-model="modPlanObj.extAcct.paidBankNum" data-vv-as="支付行号" name="paidBankNum" v-validate="modPlanObj.paymentMethod == 110300 ? 'required|numeric' : 'numeric'"></Input>
                        </FormItem>
                        </Col>
...
export default {
    ...
    data() {
        return {
            ...
            paymentList: [],   //支付额度类型
            isRequiredIcon: false,   //账务服务信息邮件接收地址是否必填
            isFax: false,   //帐单介质为传真，帐单联系人必填
            isPostCycle: true,   // 判断是否为班长权限
            acctBillingTypeArray: [],
            acctAddress: []         // 帐单寄达地址
        }
    },
    mounted() {
    ...
    watch: {
        ...
        "modPlanObj.paymentMethod"(old,newValue) {
            if(this.modPlanObj.paymentMethod == 100000) {
                ...
                this.modPlanObj.extAcct.exchgNum = ''
            }
        },
    },
    methods: {
                ...
                invFormatCustomizeRel: [
                    {
                        ...
                        objType: "1",
                        postCycle: "1",
                        postMethod: Number(this.acctAddObj.postMethod),
                        postcode: "",
                        statusCd: 1,
                        statusDate: "1",
                        updateDate: "1",
                        ...
                        billEffectiveDate: this.$utils.time2TimeStap(
