		...
		methods:{
			...
			initScroll(){
				...
				if(Math.ceil(this.srcHeight) > Math.ceil(this.maxHeight) && 
					...
					if(parseFloat(this.scrollHeightStyle.height) + parseFloat(this.scrollHeightStyle.top) > this.maxHeight){
						this.scrollHeightStyle.top = `${this.maxHeight - parseFloat(this.scrollHeightStyle.height)}px` 
					}
					...
					this.scrollStyle.paddingRight = 0;
				...
				if(Math.ceil(this.srcWidth) > Math.ceil(this.maxWidth) && 
					...
					this.scrollStyle.paddingBottom = 0;
			...
			fixedWidth(w){
				...
				for(let i = 0; i < fixedRightArray.length; i++){
					...
					if(this.srcWidth - this.maxWidth - w === 0){
						f.classList.remove('app__scroll-r-shadow');
					}else{
						f.classList.add('app__scroll-r-shadow');
					}
