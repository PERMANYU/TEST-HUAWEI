	...
	<div :class="['tree',isStart ? 'tree-start' : '']">
		...
		<Row>
			<Col :span="keyItem.span"
				...
				:key="keyIndex">
				<span :class="['first',curLevel > 6 ? 'first-7' : 'first-'+curLevel,isActive() ? 'active' : '']"
					v-if="keyIndex === 0"
					@click="firstClick()">
					{{folder[keyItem.key]}}
				...
				if(this.defaultObj.key){
					...
					if(Object.prototype.toString.call(this.value) === '[object Array]'){
						...
						for(let i = 0; i < tempEmitArray.length; i++){
							if(tempEmitArray[i][this.defaultObj.key] == this.folder[this.defaultObj.key]){
