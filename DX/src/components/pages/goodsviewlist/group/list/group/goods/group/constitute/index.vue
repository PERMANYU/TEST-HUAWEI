		...
		<div class="info-list member">
			...
			<Scroll class="app__table__inner" :isShowRight="true" ref="listScroll">
				...
				<Row class="tab-tbody" :class="['app__table-item']" v-for="(item,index) in offeProdRelArr" :key="index">
					<Col :span="3" class="col-show" :title="item.prodName">
					...
					<Col :span="2" :title="item.prodNbr">{{item.minNum}}</Col>
					<Col :span="2" :title="item.prodNbr">{{item.maxNum}}</Col>
					...
					<Row v-if="item.children.length>0" class="tab-tbody" :class="['m-item app__table-item']" v-for="(val,index) in item.children" :key="index">
						<Col :span="3"  :title="val.prodName" class="col-show">
							...
							<div class="basic-info">
								...
								<Row>
									...
									<FormItem labelText="优惠值：">
										<span>{{relsObj.discountValue=="10"?"套餐生效期间不收取月租费":relsObj.discountValue=="30"?"免一路70元（按整月减免）":""}}</span>
									</FormItem>
									</Col>
									<Col :span="7" :offset="1">
									<FormItem labelText="首月优惠值：">
										<span>{{relsObj.firstMonDiscountValue=="10"?"套餐生效前免月租":relsObj.firstMonDiscountValue=="20"?"套餐生效前租费对折":relsObj.firstMonDiscountValue=="30"?"免一路70元（按整月减免）":""}}</span>
									</FormItem>
									</Col>
									<Col :span="7" :offset="1">
							...
							<template slot="moreleafbasic">
								<div class="basic-info">
									...
									<Row>
										<Col :span="7">
										<FormItem labelText="优惠值：">
											<span>{{moreleafData.discountValue=="10"?"套餐生效期间不收取月租费":moreleafData.discountValue=="30"?"免一路70元（按整月减免）":""}}</span>
										</FormItem>
										</Col>
										<Col :span="7" :offset="1">
										<FormItem labelText="首月优惠值：">
											<span>{{moreleafData.firstMonDiscountValue=="10"?"套餐生效前免月租":moreleafData.firstMonDiscountValue=="20"?"套餐生效前租费对折":moreleafData.firstMonDiscountValue=="30"?"免一路70元（按整月减免）":""}}</span>
										</FormItem>
										</Col>
										<Col :span="7" :offset="1">
										<FormItem labelText="构成类型：">
	...
	export default {
		...
		data() {
			return {
				...
				offeProdRelArr: [{children:[{},{}],prodNbr:555}],
				prodOpenArray: [],
				statusCd: 1000,
				tabDefault: 'basic',
		...
		methods: {
			...
			getoffeProdRel() {
				...
				this.$axios.get(this.$api.replace(this.$api.GetOffeProdRel, ['{offerId}'], [this.componentData.offerId]), {
					params: {
						statusCd: 1000
					}
				}).then(res => {
			...
			GetOfferProdAttrValRest(val) {   
				this.$axios.get(this.$api.replace(this.$api.GetOfferProdAttrValRest, ['{offerId}', '{offerProdRelId}'], [this.componentData.offerId,val.offerProdRelId]), {
					params: {
						statusCd: 1000,
						page: 1,
						limit: 20
...
//							this.pageleafProductObj.pageCount = res.data.body.pageInfo.pageCount
			...
			//产品业务约束已选列表
			offerProdService(val) {  
				//let offerProdRelArr = []
				//offerProdRelArr.push(this.componentData.offerId)
				//offerProdRelArr.push(this.offerProdRelId)
				this.$axios.get(this.$api.replace(this.$api.OfferProdServiceRstr, ['{offerId}', '{offerProdRelId}'], [this.componentData.offerId,val.offerProdRelId]), {
					params: {
						statusCd: 1000
					}
				}).then(res => {
			...
			offeProdRelDetail(val) {
                ...
                this.$axios.get(this.$api.PutOfferProdRel.format({offerId:this.componentData.offerId,offerProdRelId:val.offerProdRelId})).then(res => {
					if(res.data.success) {
			...
			offeProdleafRelDetail() {
				this.$axios.get(this.$api.replace(this.$api.offerProdRelDetail, ['{offerId}', '{offerProdRelId}'], [this.componentData.offerId,this.offerProdChildRelId]), {}).then(res => {
					if(res.data.success) {
						if(res.data.offeProdRel) {
							//子产品详情
			...
			moreleafChange(val) {
				if(val == 'moreleafbasic') {
					...
					this.GetOfferProdAttrValRest(this.moreValue)
					this.moreleafDefault = val
				}
			},
			...
			moreProdAttr(val) {  
				this.$axios.get(this.$api.replace(this.$api.GetOfferProdAttrValRestDetail, ['{offerId}', '{offerProdRelId}', '{restrictId}'], [this.componentData.offerId,val.offerProdRelId,val.restrictId]), {}).then(res => {
					if(res.data.success) {
