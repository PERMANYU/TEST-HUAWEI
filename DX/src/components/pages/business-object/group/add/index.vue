	...
	<transition name="fade">
		<div class="add">
			<Scroll :isShowRight="true" ref="scroll">
				<div class="a-header">
					<div class="a-title">新建业务对象</div>
					<div class="a-close" @click="btnBack()"></div>
				</div>
				<div class="u-title1" v-show="componentData.a == 'show' ? false : true">
					<span class="u-t-item app__tab active">添加方式</span>
				</div>
				<div class="u-title1 add_action">
					<FormItem labelText="添加方式：" labelWidth="85px" :requiredIcon="true" :errorMessage="errors.first('busiObjType')" v-show="componentData.a == 'show' ? false : true">
						<RadioGroup  v-validate="'required'" :data="busiObjTypeArray" name="busiObjType" data-vv-as="添加方式" v-model="busiObj.busiObjType"></RadioGroup>
					</FormItem>
				</div>
				<div class="u-title1">
					...
					<Row>
						<Form labelWidth="110px">
							...
							<FormItem labelText="业务对象名称：" :requiredIcon="true" :errorMessage="errors.first('busiObjName')">
								<Input name="busiObjName" v-model="busiObj.busiObjName" v-validate="'required'" data-vv-as="业务对象名称"></Input>
							</FormItem>
							</Col>
							<Col :span="6" :offset="2" v-show="componentData.a == 'show' ? true : false">
							<FormItem labelText="业务大类：" :requiredIcon="true" :errorMessage="errors.first('busiTypeName')">
								<Select v-validate="'required'"  :isSearch="true" :isNullSel="false" @load="findBusiTypeList" @more="moreBusiTypeList"  name="busiTypeName" data-vv-as="业务大类" dataLabelStr="busiTypeName" dataValueStr="busiTypeId" :data="busiObj.busiTypeList"  v-model="busiObj.busiTypeId"></Select>
							</FormItem>
							</Col>
							<Col :span="6" :offset="2">
							<FormItem labelText="业务对象编码：" :requiredIcon="true" :errorMessage="errors.first('busiObjNbr')">
								<Input name="busiObjNbr" v-model="busiObj.busiObjNbr" v-validate="'required'" data-vv-as="业务对象编码"></Input>
							</FormItem>
							</Col>
						</Form>
					</Row>
				...
				<div class="app__table">
					...
					<Row class="app__table-item" v-for="(item,index) in busiAttrList" :key="index">
						...
						<span class="span" @click="clickDel('bottom',item)">删除</span>
						</Col>
					</Row>
					<Row class="app__data-none" v-show="!busiAttrList.length">
						<Col :span="24">
							<span>暂无数据</span>
						</Col>
					</Row>
			...
			<div class="a-operation">
				<div class="">
					<div class="btn-page-box">
						<span class="app__btn app__btn-prev" @click="btnBack()">取消</span>
						...
						<div v-if="attrValueSelected.length">
							...
							<div class="app__table">
								...
								<Row class="app__table-item" v-for="(item,index) in attrValueSelected" :key="index">
									...
									<Col :span="6" class="font-blue">
										<span  @click="deleteAttrValue(index)">删除</span>
									</Col>
								</Row>
							</div>
						</div>
			...
			<Dialog :dialogTitle="dialogTitle" :dialogSize="dialogValueSize" ref="attrSelect3" :initBtn="true">
			 ...
			 <!-- 业务大类列表 -->
			<Dialog dialogTitle="业务大类列表" ref="busiTypeList"  @confirm="addAttrSubmitDialog()" @close="addAttrCloseDialog">
				<Scroll  class="">
					<div class="search">
						<Search class="offer-sear" placeholder="请输入搜索内容" @search="busiSearchClick"></Search>
					</div>
					<div class="app__table">
							<Row class="m-title app__table-thead">
								<Col :span="5">业务大类编码</Col>
								<Col :span="5">业务大类名称</Col>
								<Col :span="5">状态</Col>
								<Col :span="5">版本号</Col>
								<Col :span="4">操作</Col>
							</Row>
							<Row class="app__table-item" v-for="(item,index) in busiObj.busiTypeList" :key="index">
								<Col :span="5">{{item.busiTypeNbr}}</Col>
								<Col :span="5">{{item.busiTypeName}}</Col>
								<Col :span="5">{{getStatus(item.statusCd)}}</Col>
								<Col :span="5">{{item.version}}</Col>
								<Col :span="4">
									<Radio  name="item" :label="item"  v-model="busiChoose" :value="item" class="checkBox"></Radio>
								</Col>
						</Row>

						<Row class="app__data-none" v-show="busiObj.busiTypeList.length == 0">
							<span>对不起，暂无数据</span>
						</Row>
					</div>
					<Pagination v-if="busiObj.busiTypeList.length" :pageIndex="pageInfo.pageIndex" :pageSize="pageInfo.pageSize" :rowCount="pageInfo.rowCount" :isCenter="true" @change="pageBusiChange"></Pagination>
				</Scroll>
			</Dialog>
...
export default {
    ...
    methods: {
        ...
        findAttrValueType(value) {
            //查找属性值分类
            let attrValueType = this.busiTypeArray.filter(
        ...
        findBusiTypeList(val, page) {
            this.pageInfo.pageIndex = page;
            this.$axios
                .get(this.$api.BusiType, {
                    params: {
                ...
                .then(res => {
                    if (res.data.success) {
                        this.busiObj.busiTypeList =
                            res.data.busiTypePage.content;
                        this.pageInfo = {
        ...
        moreBusiTypeList() {
            this.$refs.busiTypeList.show();
        },
        Save() {
            let parBusiObjId = null;
            if (this.busiObj.busiObjType == "-1") {
                parBusiObjId = this.componentData.parBusiObjId;
            } else if (this.busiObj.busiObjType == "1") {
                parBusiObjId = this.componentData.busiObjId + 1;
            } else {
                parBusiObjId = 0;
            }
            let dOrdBusiObjAttrDetails = [...this.busiAttrList];
            this.$axios
                .post(this.$api.AddOrdBusiObjAttrDetail, {
                    //业务对象保存
                    actType: "ADD",
                    applyRegionId: 0,
                    busiObjDesc: this.busiObj.busiObjDesc,
                    busiObjId: this.busiObjId,
                    busiObjName: this.busiObj.busiObjName,
                    busiObjNbr: this.busiObj.busiObjNbr,
                    busiObjPublishId: parBusiObjId,
                    busiTypeId: this.busiObj.busiTypeId
                        ? this.busiObj.busiTypeId
                        : this.componentData.busiTypeId,
                    createDate: "2018-11-26T02:05:07.396Z",
                    createStaff: 0,
                    ordBusiObjAttrDetails: dOrdBusiObjAttrDetails,
                    parBusiObjId: parBusiObjId,
                    remark: "",
                    statusCd: "1000",
        ...
        busiSearchClick(data) {
            this.findBusiTypeList(data);
        },
        clickDel(type, item) {
        ...
        tipConfirm() {
            if ((this.tipInfo.msg = "确认要删除选择的记录吗？")) {
                this.busiAttrList = this.busiAttrList.filter(
                    item => item.actType != "DEL"
                );
                this.checkboxData = [];
            }
            this.$refs.tip.hide();
        ...
        editObjectAttr() {},
        pageBusiChange(index) {
            this.findBusiTypeList("", index);
        },
        pageChange(index) {
        ...
        addAttrSubmitDialog() {
            //业务大类保存
            this.$set(this.busiObj, "busiTypeId", this.busiChoose.busiTypeId);
            this.$refs.busiTypeList.hide();
        },
        addAttrCloseDialog() {
            //业务大类列表回掉
        },
        btnNext() {
            ...
            if (this.busiRadio.attrValueType == "2000") {
                this.$axios
                    .get(
                        this.$api.replace(
                            this.$api.AttrSpecMore,
                            ["{attrId}"],
                            [this.busiRadio.attrId]
                        ),
                        {
                            params: {
                    ...
                    .then(res => {
                        if (res.data.success == true) {
                            console.log(res);
                            this.attrValue =
                                [...res.data.attrSpecDetail.attrValueList] ||
                                [];
                        } else {
                            this.$message.error({ message: "查询失败" });
                        }
            ...
            this.$validator.validateAll("").then(result => {
                if (result) {
                    if (this.busiRadio.attrValueType == "1000") {
                        ...
                        this.busiAttrList.push(objValue);
                        ...
                        this.attrValueSelected.forEach(element => {
                            ...
                            this.ordBusiObjAttrValues.push(obj);
                        });
                        let objValue = {
                            actType: "ADD",
                            applyRegionId: 0,
                            ...
                            isNullable: this.busiRadio.isNullable,
                            isUnique: this.busiRadio.isUnique,
                            ordBusiObjAttrValues: this.ordBusiObjAttrValues,
                            remark: "string",
                            statusCd: "1000",
                            statusDate: "2018-11-26T02:05:07.396Z",
                            tabId: 0,
        ...
        btnSave2() {
            this.busiAttrList.splice(this.delIndex, 1);
            if (this.busiRadio.attrValueType == "1000") {
                let objValue = {
                    actType: "ADD",
                    applyRegionId: 0,
        ...
        addValue(val) {
            //属性值点击
            this.attrValueSelected.filter(
                item => item.attrValueId == val.attrValueId
            )[0]
                ? (this.attrValueSelected = this.attrValueSelected.filter(
                      item => item.attrValueId != val.attrValueId
                  ))
...
.first-step {
    ...
    .search {
        ...
        .s-span {
            ...
            width: 50px;
            height: 30px;
            background: $mainColor url(images/search-btn.png) no-repeat center
                center;
            position: absolute;
            top: 0;
            right: 0;
