<template>
    <div class="main-wrap">
        <div class="require-state">
            <Row class="search-row">
                <Col :span="3" class="require-statelabel">
                    <i class="icon-all"></i>
                    <i :class="['icon-down',downFlag?'active':'']" @click="toggleDown()"></i>
                    <DropSel :obj="tabs" :text="tabInfo.label" @change="dropChange"></DropSel>
                </Col>

                <Col :span="5">
                    <Search placeholder="请输入销售品内部名称或内部编码" @search="search" ref="searchInput"></Search>
                </Col>

                <Col :span="2" :offset="14" class="btn-box">
                    <Menubtn @next="goCommodity"  :checkArr="checkArr"></Menubtn>
                </Col>
            </Row>
        </div>


        <div class="require-data">
            <keep-alive>
                <component :is="currentView" v-if="currentView = 'havedone'" ref="haveDone" :text="tabInfo.label" @checkArr="addCheckArr" @goCommodity="goCommodity" ></component>
            </keep-alive>
        </div>
    </div>
</template>

<script>
    export default {
        data() {
            return {
                currentView:'havedone',
                tabs: [
                    {
                        label:'我的销售品',
                        value:''
                    },
                ],
                downFlag:false,
                tabInfo:{
                    label:'我的销售品',
                    value:''
                },
                checkArr:[],
                
            }
        },
        methods: {
            dropChange(val) {
                this.tabInfo = val;
            },
            //跳转配置页面
            goCommodity(val) {
                val.statusCd == '1200' ? 
                this.$emit('componentView','commodity',val)
                :this.$emit('componentView','goodsview',val)
            },
            search(val) {
                this.$refs.haveDone.search(val)
            },
            toggleDown() {
                this.downFlag = !this.downFlag
            },
            addCheckArr(val) {
                this.checkArr = val
            }
        },
        components: {
            Menubtn: () => import('@/components/pages/offer-supplementary/group/list/group/menubtn'),
            havedone: () => import('@/components/pages/offer-supplementary/group/list/group/havedone'),
        }
    }
</script>

<style lang="scss" scoped>
    .main-wrap {
        height: 100%;
        padding: 0 20px 50px;
        .require-state {
            height: 50px;
            line-height: 50px;
            padding-top: 5px;
            font-size: 14px;
            color: #333;
            position: relative;
            z-index: 3;
            .require-statelabel {
                float: left;
                height: 30px;
                line-height: 30px;
                padding: 5px 0 0 20px;
                .icon-all {
                    position: absolute;
                    display: block;
                    width: 18px;
                    height: 18px;
                    margin-top: 6px;
                    margin-right: 8px;
                    background: url(images/all.png) no-repeat center;
					background-size: 18px 18px;
                }
                .drop-sel {
                    min-width: 100px;
					padding: 0 20px 0 20px;
				}
				.alllabel {
					display: inline-block;
					float: left;
					margin-right: 6px;
					cursor: pointer;
				}
                .icon-down {
                    position: absolute;
                    display: block;
                    width: 12px;
                    height: 12px;
                    margin-top: 10px;
                    left: 110px;
                    background: url(images/down.png) no-repeat center;
					background-size: 10px 6px;
                }
                .down-menu {
					position: absolute;
					top: 40px;
					left: 0;
					z-index: 2;
					width: 100%;
					background-color: #fff;
					display: none;
					border: 1px solid #F3F3F3;
					border-radius: 5px;
					.tab {
						display: block;
						float: left;
						width: 100%;
						height: 30px;
						margin-top: 10px;
						text-decoration: none;
						line-height: 30px;
						color: #323232;
						text-align: center;
						&:hover {
							background-color: #dff1fc;
						}
					}
					&.active {
						display: block;
					}
				}
            }
        }
        .require-data {
			height: 100%;
		}
    }
</style>