                ...
                <Scroll :isShowBottom="true" fixedTopClassName="app__table-thead" fixedRightClassName="app__table-right" scrollWidth="12px"  ref="scroll">
                        <div class="app__table" :class="isNoExistVaild(allRelyProdList) ? '' : 'height'">
                            ...
                            <Row  class="app__table-item" v-for="(item,index) in allRelyProdList" :key="index" v-if="item.actType != 'DEL'" :inline="true">
                                ...
                                <Col :width="250">
                                	{{getPaymentMode(item.aPaymentMode)}}
                                </Col>
                                <Col :width="200">
                                	{{getCustType(item.aCustType)}}
                                </Col>
                                <Col :width="200">
                                	{{item.aFollowProdName}}
                                </Col>
                                <Col :width="200">
                                    <span class="s-input-block">{{item.zProdName ? item.zProdName : '无'}}</span>
                                </Col>
                                <Col :width="250">
                                	{{getPaymentMode(item.zPaymentMode)}}
                                </Col>
                                <Col :width="200">
                                	{{getCustType(item.zCustType)}}
                                </Col>
                                <Col :width="200">
                                	{{item.zFollowProdName}}
                                </Col>
                                <!-- <Col :width="100">
                                ...
                                <Col :width="100">
                                	{{item.rstrType == '1000' ? '默认约束' : item.rstrType == '2000' ? '必选约束' : item.rstrType == '3000' ? '可选约束' : ''}}
                                </Col>
                            </Row>
			...
			<template>
				...
				<div class="app__table">
					...
					<Row class="app__table-item" v-for="(item,index) in numTypeRelProdList" :key="index" v-if="item.actType != 'DEL'">
						<Col :span="6">{{componentData.prodNbr ? componentData.prodNbr : '空'}}</Col>
						<Col :span="6">{{componentData.prodName ? componentData.prodName : '空'}}</Col>
						<Col :span="6">
							{{item.numType == '1000' ? '业务号码' : item.numType == '2000' ? '账号' : ''}}
						</Col>
						<Col :span="6">
							{{item.isSelectFlag == '1' ? '是' : item.isSelectFlag == '0' ? '否' : ''}}
						</Col>
					</Row>
