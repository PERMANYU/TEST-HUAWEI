<template>
	<div class="add-info">
		<Scroll :isShowRight="true">
			...
			<div class="part">
				<div class="configuration">
						...
						<div class="configuration-add" @click="addOffer()">
							<i class="add-icon"></i>
					...
					<div class="members-content">
						<div class="app__table__inner" fixedTopClassName="app__table-thead">
							<Row class="app__table-thead">
								<Col :span="11">销售品内部名称</Col>
								<Col :span="5">内部编码</Col>
								<Col :span="5">销售品类型</Col>
								<Col :span="3">操作</Col>
							</Row>

							<Row :class="['app__table-item']" v-if="offerList.length>0" v-for="(item,index) in offerList" :key="index">
								<Col :span="11" class="over-col" :title="item.offerSysName">{{item.offerSysName}}</Col>
								<Col :span="5">{{item.innerNbr}}</Col>
								<Col :span="5">{{item.offerTypeName}}</Col>
								<Col :span="3">
									<span class="del" @click="delOffer(item)"></span>
								</Col>
							</Row>
		...
		<Dialog dialogTitle="添加销售品" :dialogSize="dialogSize" class="h-dialog" ref="offerDialog" @confirm="offerDialogSubmit">
			<Scroll :isShowRight="true">
				<div class="part">
					<div class="members">
						<div class="members-search">
							<i class="iconfont icon-sousuo1130"></i>
							<Input placeholder="请输入销售品内部名称" v-model="offers.searchVal"></Input>
							<span class="members-go" @click="searchOffer()"></span>
						</div>
						<div class="members-content">
							<div class="app__table__inner" fixedTopClassName="app__table-thead"  ref="offerScroll">
								<Row class="app__table-thead">
									<Col :span="13">销售品内部名称</Col>
									<Col :span="7">内部编码</Col>
									<Col :span="4">操作</Col>
								</Row>

								<Row :class="['app__table-item']" v-if="allOffers.length>0" v-for="(item,index) in allOffers" :key="index">
									<Col :span="13" class="over-col" :title="item.offerSysName">{{item.offerSysName}}</Col>
									<Col :span="7">{{item.innerNbr}}</Col>
									<Col :span="4">
										<input type="checkbox" @click="selectOffer(index,$event)" v-bind:checked="selectMkt(item)">
									</Col>
								</Row>
								<Row v-if="!allOffers.length" class="app__data-none">
									<Col :span="24"><span>当前无数据!</span></Col>
								</Row>
							</div>
						</div>
						<div class="members-footer" v-if="allOffers.length>0">
							<Pagination :pageIndex="offers.pageInfo.pageIndex" :pageSize="offers.pageInfo.pageSize" :rowCount="offers.pageInfo.rowCount" :perPages=5 @change="offersPageChange">
							</Pagination>
						</div>
						...
						<div class="members-content">
							<div class="app__table__inner" fixedTopClassName="app__table-thead"  ref="listScroll">
								...
								<Row :class="['app__table-item']" v-if="offers.selectedOffers.length>0" v-for="(item,index) in offers.selectedOffers" :key="index">
									<Col :span="13" class="over-col" :title="item.offerSysName">{{item.offerSysName}}</Col>
									<Col :span="7">{{item.innerNbr}}</Col>
									<Col :span="4">
										<span class="del" @click="delSelectedOffer(index)"></span>
									</Col>
								</Row>
								<Row v-if="!offers.selectedOffers.length" class="app__data-none">
									<Col :span="24"><span>当前无数据!</span></Col>
								</Row>
							</div>
						</div>
			...
			initOffers(){//获取已有销售品成员
				this.$axios.get(this.$api.OfferGroupMember,{
					 params:{
						...
						// offerSysName: this.offers.searchAddedVal
...
                        })
			...
			addOffer(){//添加销售品
				...
				this.offers.pageInfo.pageIndex = 1;
				this.getOffers() 
				this.$refs.offerDialog.show()
			},
			getOffers(){//获取销售品
				this.$axios.get(this.$api.OfferGetList,{
                    params:{
						page: this.offers.pageInfo.pageIndex,
						limit: this.offers.pageInfo.pageSize,
                    ...
                    if (res.data.success) {
						this.allOffers = res.data.offerPage.content||[];
						this.offers.pageInfo.pageCount = Math.ceil(res.data.offerPage.total/this.offers.pageInfo.pageSize)
						this.offers.pageInfo.rowCount = res.data.offerPage.total
                    }else{
                        this.$message.error({message: res.data.msg})
                    }
                }).catch( error => {
			...
			offersPageChange(page){//所有销售品换页
				this.offers.pageInfo.pageIndex = page;
				this.$nextTick(() => {
					this.getOffers();
				})
			},
			addedOfferPage(page){//已选销售品换页
			...
			selectOffer(index,event){//选择需要添加的销售品
				let $target = event.target
				if($target.checked){
					if(!this.offers.selectedOffers.find(item => item.offerId == this.allOffers[index].offerId)){
						this.offers.selectedOffers.push(this.allOffers[index])
					}
				}else{
					let idx = 0
					for(let i = 0;i < this.offers.selectedOffers.length; i++){
						if(this.allOffers[index].offerId == this.offers.selectedOffers[i].offerId){
							idx = i
						}
					}
					this.offers.selectedOffers.splice(idx, 1)
				}
			},
			selectMkt(val){
                return this.offers.selectedOffers.find(item => item.offerId == val.offerId)
            },
			selectGrpMkt(val){
                return this.group.selectedGroups.find(item => item.offerGrpId == val.offerGrpId)
            },
			offerDialogSubmit(){//保存已选择的销售品
				let memberParams = []
				this.offers.submitOffers = this.offers.selectedOffers
				if(this.offers.submitOffers.length && this.offerList.length){
					for(let i = 0;i < this.offers.submitOffers.length;i++){
						for(let j = 0;j < this.offerList.length;j++){
							if(this.offers.submitOffers[i].offerId == this.offerList[j].offerId){
								this.offers.submitOffers.splice(i, 1)
							}
						}
					}
				}
				if(this.offers.submitOffers.length){
					for(let i = 0;i < this.offers.submitOffers.length;i++){
						let member = {
							offerGrpId:  this.componentData.offerGrpId,
							...
							offerId: this.offers.submitOffers[i].offerId
						}
						memberParams.push(member)
					}
					this.$axios.post(this.$api.OfferGrpMemberTask,memberParams).then(res=>{
						if (res.data.success) {
							this.$message.success({ message: '添加成功！' })
							this.$nextTick(() => {
								this.initOffers()
							})
							this.$refs.offerDialog.hide()
			...
			delSelectedOffer(index){//删除已选择的销售品成员
				this.offers.selectedOffers.splice(index, 1)
			},
...
                }
			...
			delOffer(item){//删除已有销售品成员
				this.$axios.delete(this.$api.OfferGrpMemberTask,{
					params: {
						offerGrpMemberId: item.offerGrpMemberId,
						updateStaff: this.$user.getInfo().id
					}
				}).then(res => {
					if (res.data.success) {
