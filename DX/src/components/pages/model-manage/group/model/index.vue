            ...
            <Scroll class="app__table" :isShowRight="true" fixedTopClassName="m-title" ref="listScroll">
                <DragGroup class="m-title app__table-thead" :titleList="tableTitleData" @dragchange="dragchange">
                    ...
                    <DragItem :class="['m-item']" :span="item.span" :value="item" v-for="(item,index) in tableTitleData" :key="'table'+index">
                        <span>{{item.name}}</span>
                        <ColSel :class="item.param != 'updateTime' ? 'col-search' : 'col-none'" :showOrder="false" @confirm="Confirmfilter(item)">
                            <Input v-if="item.param == 'modelName' || item.param == 'modelKey'" :placeholder="'请输入'+ item.name" v-model="item.part"></Input>
                ...
                <Row class="app__table-item" v-for="(item,index) in tabList" :key="index">
                    ...
                    <span v-if="titleItem.param != 'updateTime'" :class="[titleItem.param == 'modelName' ? 'font-blue' : '']">{{adjustTabelItemText(item,titleItem.param)}}</span>
                    <span v-else>{{adjustTabelItemText(item,titleItem.param) | date}}</span>
                    </Col>
                    <Col :span="3" class="font-blue">
                    <span class="span" @click="design(item)">编辑</span>
                    <span class="span" @click="deploy(item)">部署</span>
...
export default {
    ...
    methods: {
        ...
        deployChange() {
            this.$axios
                .post(
                    this.$api.DeployModel +
                        `?centerId=${
                            this.deployObj.centerId
                        }&businessTypeCode=${
                            this.deployObj.businessTypeCode
                        }&modelId=${this.deployObj.modelId}`,
                    {
                        centerId: this.deployObj.centerId,
                        businessTypeCode: this.deployObj.businessTypeCode,
                        modelId: this.deployObj.modelId
                    },
                    this.headers
                )
            ...
            this.$axios
                ...
                .then(res => {
                    if (res.data.success) {
                        ...
                        if (type) {
                            window.open(
                                `http://10.145.208.183:5600/editor/cpcmodel/${type}/${bpmsTenantId}/${bpmsAuthCode}`
                            );
                        } else {
                            this.getModelList(this.searchData);
        ...
        dragchange() {},
        tipConfirm() {
            if (this.tipType == "deploy") {
                this.deployChange();
            } else if (this.tipType == "delModel") {
                this.delimiters(this.delObj);
            } else if (this.tipType == "delModels") {
