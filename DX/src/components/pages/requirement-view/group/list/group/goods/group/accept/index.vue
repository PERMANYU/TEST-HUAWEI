		...
		<div class="i-content">
			...
			<Form labelWidth="120px" labelAlign="left">
				...
				<Row>
					...
					<Col :span="6" :offset="1" v-show="offerOrderRule.effType == '1600'">
						<FormItem labelText="指定生效时间：">
							<span>{{offerOrderRule.effDate | date}}</span>
						</FormItem>
					</Col>
					<Col :span="6" :offset="1"  v-show="offerOrderRule.expType == '1600'">
...
<script type="text/javascript">
	export default {
		...
		methods: {
			getRuleData() {
				this.$axios.get(this.$api.OfferGetList+'/'+this.componentData.offerId+'/offerRule', {
					params: {
						statusCd:1000
					}
				}).then(res => {
					if(res.data.success == true) {
						console.log(res,'受理')
						this.offerOrderRule = res.data.offerOrderRule;
						this.offerRemewalRule = res.data.offerRemewalRule;
						this.offerUnsubscribeRule = res.data.offerUnsubscribeRule;
						this.offerChangeRule = res.data.offerChangeRule;
						this.offerExpCfg = res.data.offerExpCfg;
