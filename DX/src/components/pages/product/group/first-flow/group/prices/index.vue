<template>
	<div class="prices">
		<Scroll :isShowRight="true" ref="scroll">
			...
			<div class="selectbtn app__btn app__btn-add" @click.stop.self="selectPrimaryClick()">添加一次性费用模板</div>
			<div class="a-v-table app__table">
				...
				<Row :class="['v-t-item app__table-item',(index + 1) % 2 === 0 ? 'double' : 'single']" v-for="(item,index) in adjustPriceList()" :key="index">
					...
					<Col :span="2" class="local-icon">
						<span @click.stop.self="delPrimary(item)">删除</span>
					</Col>
				</Row>
			...
			<div class="selectbtn app__btn app__btn-add" @click.stop.self="selectPeriodicClick">添加周期性模版</div>
			<div class="a-v-table app__table">
				<Row class="app__table-thead">
					<Col :span="6">模板名称</Col>
