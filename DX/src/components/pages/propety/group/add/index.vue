	...
	<transition name="fade">
		<div class="add">
			<Scroll :isShowRight="true" ref="scroll">
				...
				<div class="a-value" v-if="step=='1000'">
					...
					<div class="a-info">
						<Row>
							<Form labelWidth="110px">
								<Col :span="6">
                                    <FormItem labelText="业务大类：" :requiredIcon="true" :errorMessage="errors.first('busiTypeId')">
								...
								<Col :span="6">
                                    <FormItem labelText="属性格式：" :requiredIcon="false" :errorMessage="errors.first('attrFormat')">
								...
								<Col :span="6" >
                                    <FormItem labelText="结束值：" :requiredIcon="false" :errorMessage="errors.first('valueTo')">
								...
								<Col :span="6" :offset="2">
                                    <FormItem labelText="起始值：" :requiredIcon="false" :errorMessage="errors.first('valueFrom')">
					...
					<div class="a-info" v-if="attrForm.attrValueType == '1000'">
						<Row >
							<Form labelWidth="110px">
								<Col :span="6">
                                    <FormItem labelText="属性值：" :errorMessage="errors.first('defaultValue')">
				...
				<!-- <div class="a-complete" v-if="step == 3">完成</div> -->
			</Scroll>
			...
			<Dialog dialogTitle="选择业务大类" :dialogSize="dialogSize" ref="busiTypeDialog" @confirm="confirmBusiType()">
				...
				<Row class="h-d-item" :class="['m-item app__table-item']" v-for="(item,index) in busiTypeData" :key="index">
...
<script type="text/javascript">
	export default {
		...
		//                        updateStaff: this.staffId,                           //修改人id
            ...
            // //保存属性值(第二步)
            // setAttrVal(){
            //     this.$axios.post(this.$api.replace(this.$api.OrdAttrValue,['{attrId}'],[this.attrId]), {
            //         ordAttrValueList: this.attrValData
            //     }).then(res => {
            //         if(res.data.success == true) {
            //             this.$message.success({ message: '保存成功！'});
