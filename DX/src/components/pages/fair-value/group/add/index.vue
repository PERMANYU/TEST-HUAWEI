<template>
    <transition name="fade">
		<div class="product">
			...
			<div class="title">
				<span class="t-name">添加公允价值</span>
				<span class="close" @click="pageClose()"></span>
			</div>
			<!-- 步骤条 -->
			<div class="step-item">
				<span :class="['i-name', this.tabFlag == 1 ?'active' : '' ] "> ① 填写基本信息 
					<span class="before"> </span>
				</span>
				<span :class="['i-name', this.tabFlag == 2 ?'active' : '' ]">② 配置公允值价值规则
					<div class="after"></div>
				</span>
			</div>
						...
						<Form labelWidth="120px" labelAlign="right">
							<Col :span="8">
								<FormItem labelText="业务名称:" :requiredIcon="true" :errorMessage="errors.first('fairValueName')">
									<Input v-validate="'required'" v-model.trim="fairObj.fairValueName"  name="fairValueName" data-vv-as="业务名称"></Input>
								</FormItem>
							</Col>
							<Col :span="7" :offset="1" >
								<FormItem labelText="所属目录:" :requiredIcon="true" :errorMessage="errors.first('catalogItemName')">
									<Input v-validate="'required'" name="catalogItemName" data-vv-as="所属目录" v-model="fairObj.catalogItemName" @click.native="catalogClick()" ref="catalogItemNameRef"></Input>
								</FormItem>
							</Col>
							<Col :span="7" :offset="1">
								<FormItem labelText="类型:" :requiredIcon="true" :errorMessage="errors.first('bizType')">
									<Select v-validate="'required'" :data="fairTypeArray"  v-model="fairObj.bizType" name="bizType" data-vv-as="类型" :isNullSel="false"></Select>
								</FormItem>
							</Col>
							<Col :span="8">
								<FormItem labelText="公允价值(元):" :requiredIcon="true" :errorMessage="errors.first('fairValue')">
									<Input v-validate="'required'" v-model.trim="fairObj.fairValue"  name="fairValue" data-vv-as="公允价值(元)" number></Input>
								</FormItem>
							</Col>

							<Col :span="7" :offset="1">
								<FormItem labelText="公允价值单位:" :requiredIcon="true" :errorMessage="errors.first('unit')">
									<Select v-validate="'required'" :data="fairValueArray"  v-model="fairObj.unit" name="unit" data-vv-as="类型" :isNullSel="false"></Select>
								</FormItem>
							</Col>
			...
			<div class="access-config" v-if="this.tabFlag === 2" ref="addAttrValue">
				...
				<div class="basic-info" v-if="this.fairObj.bizType == '1'" ref="listScroll" :isShowRight="true">
					<!-- 产品配置 -->
					<div class="main">
						<div class="c_title">
						...
						<div class="p-footer" v-if="this.fairObj.bizType == '1'">
					...
					<div class="main main-funfee">
						...
						<Scroll class="app__table__inner" fixedTopClassName="app__table-thead" :isShowRight="true" ref="afListScroll">
							...
							<Row :class="['app__table-item']" v-for="(item,index) in functionFeeConfigList" :key="index">
								...
								<Col :span="5">
									<Select :data="funFeeConfigPro"  dataValueStr="objId" dataLabelStr="prodName" v-model="item.prodName" name="prodName" data-vv-as="产品" :isNullSel="false" @change="funFeeConfigProChange(arguments[0], item)"></Select>
								</Col>
								<Col :span="5">
									<Select multiple  @change="prodActChange(arguments[0],item)" :data="funFeeWay"  v-model="item.payType" name="payType" data-vv-as="付费方式" :isNullSel="false" class="funfee-select"></Select>
								</Col>
								<Col :span="2">
				...
				<!-- 通信类 -->
				<div class="basic-info" v-if="this.fairObj.bizType == '2'" ref="listScroll" :isShowRight="true">
					<!-- 资费模板配置 -->
					<div class="main">
						<div class="c_title">
					...
					<div class="main main-funfee">
						...
						<div class="p-footer" v-if="this.fairObj.bizType == '2'">
					...
					<Scroll  :isShowBottom="true" v-if="funProSelected.length>0" ref="yxOfferRelsScroll" >
						<div class="app__table__inner">
							...
							<Row :class="['app__table-item']"  v-for="(item,index) in funProSelected" :key="index">
								<Col :span="6" :title="item.prodName" class="no-wrap">{{item.prodName}}</Col>
        ...
        methods:{
			...
			catalogClick() {
			 ...
			 catalogItemDialogConfirm() {
                ...
                this.fairObj.catalogItemName = this.catalogItemTreeModel.catalogItemName;
				this.fairObj.catalogItemId = this.catalogItemTreeModel.catalogItemId;
			...
			// 产品配置列表
			getProductList() {
				this.$axios.get(this.$api.QueryProductList, {
					params: {
					...
					if (res.data.success === true) {
						...
						// 产品下拉框
						this.funFeeConfigPro = [...res.data.fairobjproductlist.content];
					} else {
						this.$message.error({"message": "查询失败!"})
					}
				}).catch(error => {
					this.$message.error({"message": "服务器异常--产品配置列表!"})
				})
			},
			// 收入费用项配置列表
			getFairChrgRelList() {
				this.$axios.get(this.$api.replace(this.$api.QueryFairChrgRelList, ['{fairValueId}'], [this.fairValueId]), {
					params: {
						fairValueId: this.fairValueId,
					...
					this.$message.error({"message": "服务器异常--收入费用项配置列表!"})
				})
			},
			// 添加产品配置列表
			getProduct() {
			...
			addFunFeePage(item) {
				...
				// } else {
					let Temp = {
						...
						prodId: item.prodId, // 收入费用-产品
						payType: item.payType // 收入费用-付费方式
					}
			...
			addFunProConfigSubmit() {
				...
				if (this.isOk) {
					...
					this.$axios.put(this.$api.replace(this.$api.AddProduct, ['{fairValueId}'], [this.fairValueId]), {
			...
			addFunFeeConfigSubmit() {
				...
				if (this.isOk) {
					...
					this.$axios.put(this.$api.replace(this.$api.AddFeeItem, ['{fairValueId}'], [this.fairValueId]), {
			...
			funFeeConfigProChange(arg, item) {
			},
			...
			selectMktTariffPro(val) {
				return this.tariffProSelected.find(item => item.prodId == val.prodId)
			},
			// 是否选中(收入费用)
			...
			deleteTariffProClick(item) {
				for(var i=0;i<this.tariffProSelected.length;i++){
					if(this.tariffProSelected[i].prodId == item.prodId){
			...
			addTariffProFieldSubmit() {
				...
				this.$validator.validateAll().then((result) => {
					if (result) {
						this.$axios.put(this.$api.replace(this.$api.AddTariff, ['{fairValueId}'], [this.fairValueId]), {
							fairValueId: this.fairValueId,
							objType: '2000',
			...
			submit() {
				this.$validator.validateAll().then((result) => {
					if (result) {
						this.$axios.post(this.$api.AddFairBase, {
							fairValueName: this.fairObj.fairValueName,
							catalogItemId: this.fairObj.catalogItemId,
							bizType: this.fairObj.bizType,
							unit: this.fairObj.unit,
							fairValue: this.fairObj.fairValue,
							createStaff: this.$user.getInfo().id      
						}).  then(res => {
							if (res.data.success === true) {
								this.$message.success({ message : "公允价值基本信息添加成功" })
								this.fairValueId = res.data.fairvalue.fairValueId;
								this.tabFlag = 2;
								if (this.fairObj.bizType == '1') {
									this.getProductList();
									this.getFairChrgRelList();
								} else {
									this.getTariffList();
			...
			submitgroup() {
				if (this.fairObj.bizType == '1') {
					...
					this.$axios.put(this.$api.replace(this.$api.EditFeeConfig, ['{fairValueId}'], [this.fairValueId]), {
